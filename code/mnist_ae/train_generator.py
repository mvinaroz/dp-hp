...
from mnist_ae.aux import rff_gauss, get_mnist_dataloaders, plot_mnist_batch
...
def test(args, enc, gen, device, test_loader, rff_mmd_loss, epoch):
  ...
  plot_mnist_batch(gen_samples[:100, ...].cpu().numpy(), 10, 10, f'samples/gen_samples_ep{epoch}.png')
...
def main():
  ...
  parser.add_argument('--d-code', type=int, default=5)
  parser.add_argument('--d-rff', type=int, default=100)
  parser.add_argument('--rff-sigma', type=int, default=2.0)
  ...
  if args.conv_ae:
    enc = ConvEncoder(args.d_enc, nc=(1, 4, 8, 16), extra_conv=True)
