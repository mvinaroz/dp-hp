...
def err_cross(k, seed):
  ...
  print(
    'Found Hermite Polynomials for set1 and set2 of data with T=' + str(T) + ' and N=' + str(N) + ' and k=' + str(k))
  # print(h1)
  # %% Eigenvalues and Eigenfunctions
  l1_nus = np.sum(nus, axis=0,
                  keepdims=True)  # The first dimension still has 1 dims. However, it could be integrated into other nus expressions.
  facts = sm.factorial(nus)
  eigv = ((xi / 2) ** (l1_nus)) / (np.prod(facts, axis=0, keepdims=True))
  eigf1 = (1 - xi ** 2) ** (k / 4) * np.expand_dims(
    np.exp(- xi / (1 + xi) * (np.linalg.norm(g1, axis=1, keepdims=True)) ** 2),
    list(np.arange(2, k + 1, dtype=int))) * h1
  eigf2 = (1 - xi ** 2) ** (k / 4) * np.expand_dims(
    np.exp(- xi / (1 + xi) * (np.linalg.norm(g2, axis=1, keepdims=True)) ** 2),
    list(np.arange(2, k + 1, dtype=int))) * h2
  # print(np.shape(eigv))
