...
def main(dataset, undersampled_rate, n_features_arg, mini_batch_size_arg, how_many_epochs_arg, is_priv_arg, seed_number):
  ...
  def save_generated_samples_with_labels(samples, labels):
    path_gen_data = f"../data/generated/{arguments.dataset}"
    os.makedirs(path_gen_data, exist_ok=True)
    if is_private:
      filename = f"{arguments.dataset}_generated_privatized_{is_private}_eps_{epsilon}_epochs_{arguments.epochs}_features_{arguments.num_features}_samples_{samples.shape[0]}_features_{samples.shape[1]}"
      np.savez(os.path.join(path_gen_data, filename), x=samples, y=labels)
    else:
      filename = f"{arguments.dataset}_generated_privatized_{is_private}_epochs_{arguments.epochs}_features_{arguments.num_features}_samples_{samples.shape[0]}_features_{samples.shape[1]}"
      np.savez(os.path.join(path_gen_data, filename), x=samples, y=labels)
    print(f"Generated data saved to {path_gen_data}")
  ...
  if arguments.data_type=='generated' or arguments.data_type=='both':
    ...
    if dataset in heterogeneous_datasets:
      ...
      if arguments.save_generated:
        save_generated_samples_with_labels(generated_input_features_final, generated_labels_final)
